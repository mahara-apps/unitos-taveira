...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Upsert canonical memory (INSERT ... ON CONFLICT DO UPDATE)
      INSERT INTO public.brain_memory(
        ...
        source_event, source_refs, origin, status, tags, metadata,
      ...
      DO UPDATE SET
        ...
        updated_at          = now(),
        version             = public.brain_memory.version + 1,
        ...
        content = jsonb_set(
                    jsonb_set(
                      jsonb_set(public.brain_memory.content,
                        '{last_action}', to_jsonb(v_row.action), true),
                      '{last_event_id}', to_jsonb(v_row.id), true),
                    '{occurrence_count}',
      ...
      INSERT INTO public.brain_memory_versions(
        memory_id, version, confidence, previous_confidence, delta_confidence,
        title, description, status, change_reason, source_event
      )
      SELECT m.id, m.version, m.confidence, v_prev_conf,
             (m.confidence - COALESCE(v_prev_conf, m.confidence)),
             m.title, m.description, m.status,
             'learning:' || v_row.event_type || COALESCE(':' || v_row.action, ''),
             v_row.id
      ...
      -- Insights (mesmas regras anteriores)
